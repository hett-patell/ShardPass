...
export const items: VaultItem[] = [
  {
    ...
    notes: "Work account. Recovery codes in the safe.",
    favorite: true,
    tags: ["work"],
  ...
  },
  {
    ...
    kind: "login",
    name: "Google",
    username: "het@example.com",
    password: "p4ssw0rd-google",
  ...
  },
  {
    ...
    kind: "login",
    name: "Proton Mail",
    username: "het@proton.me",
    password: "proton-pass",
  ...
  },
  {
    ...
    kind: "note",
    name: "Home Wi-Fi",
    content: "SSID: Bramble\nPassword: sunflower-42\n\nGuest network is open on weekends.",
    tags: ["home"],
  },
  {
    ...base,
...
function samplePassword(request: {
  mode?: string;
  length?: number;
  wordCount?: number;
  separator?: string;
  ...
  if (request.mode === "passphrase") {
    const sep =
      { hyphen: "-", space: " ", period: ".", none: "" }[request.separator ?? "hyphen"] ?? "-";
    const picked = Array.from(
      { length: request.wordCount ?? 4 },
      (_, i) => words[(i * 3 + Date.now()) % words.length] ?? "ember",
    );
    return {
      password: picked
        .map((w) => (request.capitalize ? w[0]!.toUpperCase() + w.slice(1) : w))
        .join(sep),
      entropyBits: 12.9 * (request.wordCount ?? 4),
    };
  }
  ...
  for (let i = 0; i < length; i += 1)
    out += alphabet[(i * 7 + Date.now() + i * i) % alphabet.length];
...
export function createScriptedPlatform(options: { tabUrl?: string; scenario?: Scenario } = {}) {
  ...
  const vaultState =
    scenario === "setup" ? "unconfigured" : scenario === "locked" ? "locked" : "unlocked";
  ...
  return {
      ...
      switch (request.kind) {
        ...
        case "item.get":
          return Promise.resolve({
            version: 1,
            kind: "item.getResult",
            item: visible.find((item) => item.id === request.itemId),
          });
            ...
            setTimeout(
              () =>
                resolve({
                  ...
                  ...samplePassword(request as Parameters<typeof samplePassword>[0]),
                }),
              120,
            ),
          );
        ...
        case "folder.list":
          return Promise.resolve({
            ...
            folders: [
              { id: id(901), name: "Work" },
              { id: id(902), name: "Clients", parentId: id(901) },
              { id: id(903), name: "Home" },
            ],
          });
        case "vault.lock":
    ...
    sendOtpMessage(request: OtpRequest): Promise<OtpResponse> {
      if (request.kind === "otp.getCode" || request.kind === "otp.copyCode") {
        ...
        return Promise.resolve({
          version: 1,
          kind: "otp.codeResult",
          itemId: (request as { itemId: string }).itemId,
          revision: 1,
          code: "482913",
          otpType: "totp",
          period: 30,
      ...
      if (request.kind === "otp.list")
        return Promise.resolve({ version: 1, kind: "otp.listResult", items: [] } as OtpResponse);
    ...
    sendEnteMessage(): Promise<EnteSafeState> {
      return Promise.resolve({
        ...
        pendingCount: 0,
        conflictCount: 0,
        lastSuccessAt: Date.now() - 4 * 60_000,
        nextEligibleAt: Date.now() + 11 * 60_000,
    ...
    activeTab: () =>
      Promise.resolve(options.tabUrl === undefined ? null : { id: 1, url: options.tabUrl }),
    sendToTab: () =>
      Promise.resolve({ version: 1, kind: "login.fillFromPopupResult", status: "filled" }),
  };
}
