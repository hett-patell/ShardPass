...
const popupSender: SenderContext = { ...vaultSender, contextKind: "popup", senderUrl: "chrome-extension://extension-id/popup/index.html" };

const ids = ["11111111-1111-4111-8111-111111111111", "22222222-2222-4222-8222-222222222222", "33333333-3333-4333-8333-333333333333", "44444444-4444-4444-8444-444444444444"] as const;

function login(id: string, folderId?: string): VaultItem {
  return {
    id, schemaVersion: 2, revision: 1, createdAt: "2026-08-10T12:00:00.000Z", updatedAt: "2026-08-10T12:00:00.000Z",
    favorite: false, tags: [], kind: "login", name: "x", username: "u", password: "p", urls: [], notes: "",
...
function fixture(folders: Folder[] = [], items: VaultItem[] = []) {
  ...
  return { service, get stored() { return stored; }, updated };
}
...
describe("FolderService", () => {
  it("creates, renames, and lists folders", async () => {
    ...
    const child = await service.handle(req("folder.create", { name: "Clients", parentId: ids[0] }), vaultSender);
    expect(child.folders[1]).toEqual({ id: ids[1], name: "Clients", parentId: ids[0] });
    const renamed = await service.handle(req("folder.rename", { id: ids[0], name: "Office" }), vaultSender);
  ...
  it("refuses a sibling with the same name, ignoring case", async () => {
    ...
    await expect(service.handle(req("folder.create", { name: "work" }), vaultSender)).rejects.toMatchObject({ code: "FOLDER_INVALID" });
  });
  ...
  it("refuses nesting beyond the depth limit", async () => {
    ...
    await expect(service.handle(req("folder.create", { name: "d", parentId: ids[2] }), vaultSender)).rejects.toMatchObject({ code: "FOLDER_INVALID" });
  });

  it("deleting a folder removes its subtree and un-files the items inside, deleting none", async () => {
    const { service, updated } = fixture(
      [{ id: ids[0], name: "a" }, { id: ids[1], name: "b", parentId: ids[0] }, { id: ids[2], name: "c" }],
      [login("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", ids[1]), login("bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", ids[2])],
    );
  ...
  it("is vault-only", async () => {
    const { service } = fixture();
    await expect(service.handle(req("folder.list"), popupSender)).rejects.toMatchObject({ code: "FOLDER_INVALID" });
  });
});
