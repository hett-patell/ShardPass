...
function login(overrides: Record<string, unknown> = {}): VaultItem {
  return {
    id: LOGIN_ID, kind: "login", schemaVersion: 2, revision: 1, createdAt: stamp, updatedAt: stamp, favorite: false, tags: [],
    name: "GitHub", username: "octocat", password: "pw", urls: ["https://github.com"], notes: "", ...overrides,
  };
}
...
function fixture(items: VaultItem[] = []) {
  ...
  const repository = {
    ...
    updateItem: (candidate: VaultItem, expectedRevision: number) => {
      ...
      if (current === undefined || current.revision !== expectedRevision) return Promise.reject(new Error("conflict"));
      const next = { ...candidate, revision: expectedRevision + 1 } as VaultItem;
      store.set(candidate.id, next);
      return Promise.resolve(next);
    },
  };
  const service = new PasskeyService({ repository, now: () => 1_755_000_000_000, notePrivilegedActivity: () => Promise.resolve() });
  return { service, store };
}
const req = (kind: string, values: Record<string, unknown>) => ({ version: 1, kind, ...values }) as never;
const clientData = (type: string) =>
  new TextEncoder().encode(JSON.stringify({ type, challenge: "Y2hhbGxlbmdl", origin: "https://github.com", crossOrigin: false }));
...
describe("PasskeyService", () => {
  it("registers a passkey on the matching login and later signs an assertion the relying party can verify", async () => {
    ...
    const registered = (await service.handle(
      req("passkey.register", {
        origin: "https://github.com", rpId: "github.com", rpName: "GitHub", userHandle: "dXNlcg", userName: "octocat",
        userDisplayName: "Octo Cat", algorithms: [-8, -7, -257], excludeCredentialIds: [], clientDataJson: b64url(clientData("webauthn.create")),
      }),
      contentSender,
    )) as { itemId: string; credentialId: string; publicKey: string; authenticatorData: string; attestationObject: string };
    expect(registered.itemId).toBe(LOGIN_ID);
    const saved = store.get(LOGIN_ID) as { passkeys?: { rpId: string; userName: string; credentialId: string }[]; revision: number };
    expect(saved.passkeys).toHaveLength(1);
    expect(saved.passkeys?.[0]).toMatchObject({ rpId: "github.com", userName: "octocat", credentialId: registered.credentialId });
    expect(saved.revision).toBe(2);
    expect(fromBase64Url(registered.attestationObject)[0]).toBe(0xa3);

    const candidates = (await service.handle(
      req("passkey.candidates", { origin: "https://github.com", rpId: "github.com", allowCredentialIds: [] }),
      contentSender,
    )) as { candidates: { itemId: string; credentialId: string; userName: string }[] };
    expect(candidates.candidates).toEqual([{ itemId: LOGIN_ID, credentialId: registered.credentialId, userName: "octocat", loginName: "GitHub" }]);

    const client = clientData("webauthn.get");
    const asserted = (await service.handle(
      req("passkey.assert", { origin: "https://github.com", rpId: "github.com", itemId: LOGIN_ID, credentialId: registered.credentialId, clientDataJson: b64url(client) }),
      contentSender,
    ...
    const publicKey = await crypto.subtle.importKey("spki", Uint8Array.from(fromBase64Url(registered.publicKey)).buffer, { name: "ECDSA", namedCurve: "P-256" }, false, ["verify"]);
  ...
  it("creates a login when the site has none, refuses foreign rp ids, and honours exclude lists", async () => {
    ...
    const registered = (await service.handle(
      req("passkey.register", {
        origin: "https://accounts.example.test", rpId: "example.test", rpName: "Example", userHandle: "aWQ", userName: "me@example.test",
        userDisplayName: "", algorithms: [-7], excludeCredentialIds: [], clientDataJson: b64url(clientData("webauthn.create")),
      }),
      contentSender,
    )) as { itemId: string; credentialId: string };
    const created = store.get(registered.itemId) as { username: string; urls: string[]; passkeys: unknown[] };
    ...
    await expect(
      service.handle(
        req("passkey.register", {
          origin: "https://evil.test", rpId: "example.test", rpName: "x", userHandle: "aWQ", userName: "u", userDisplayName: "",
          algorithms: [-7], excludeCredentialIds: [], clientDataJson: b64url(clientData("webauthn.create")),
        }),
        contentSender,
      ),
    ...
    await expect(
      service.handle(
        req("passkey.register", {
          origin: "https://example.test", rpId: "example.test", rpName: "x", userHandle: "aWQ", userName: "me@example.test", userDisplayName: "",
          algorithms: [-7], excludeCredentialIds: [registered.credentialId], clientDataJson: b64url(clientData("webauthn.create")),
        }),
        contentSender,
      ),
    ...
    await expect(
      service.handle(
        req("passkey.register", {
          origin: "https://example.test", rpId: "example.test", rpName: "x", userHandle: "aWQ", userName: "other", userDisplayName: "",
          algorithms: [-257], excludeCredentialIds: [], clientDataJson: b64url(clientData("webauthn.create")),
        }),
        contentSender,
      ),
