...
describe("item lifecycle through the installed background", () => {
  it("deletes after an update (item.create path)", async () => {
    ...
    const u = await platform.dispatchMessage({ version: 1, kind: "item.update", itemId: a.id, expectedRevision: 1, fields: { notes: "edited" } }, vaultSender) as any;
    expect(u.kind).toBe("item.mutationResult");
    const d = await platform.dispatchMessage({ version: 1, kind: "item.delete", itemId: a.id }, vaultSender) as any;
    dispose();
  ...
  it("deletes after an update (createMany path)", async () => {
    ...
    const u = await platform.dispatchMessage({ version: 1, kind: "item.update", itemId: b.id, expectedRevision: 1, fields: { notes: "edited" } }, vaultSender) as any;
    expect(u.kind).toBe("item.mutationResult");
    const d = await platform.dispatchMessage({ version: 1, kind: "item.delete", itemId: b.id }, vaultSender) as any;
    dispose();
    expect(d.kind).toBe("item.deleteResult");
  });
  ...
  it("lock settings update is reflected in state and alarm", async () => {
    const { platform, dispose } = await setup();
    const r = await platform.dispatchMessage({ version: 1, kind: "vault.updateLockSettings", autoLockMinutes: 30, lockOnScreenLock: false }, vaultSender) as any;
    expect(r.kind).toBe("vault.ok");
    const s = await platform.dispatchMessage({ version: 1, kind: "vault.getState" }, vaultSender) as any;
    dispose();
