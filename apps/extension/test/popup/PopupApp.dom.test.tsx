...
function createTestPlatform(
  ...
  const sendMessage = vi.fn((payload: unknown): Promise<unknown> => {
    ...
    if (request.kind === "item.list")
      return Promise.resolve({ version: 1, kind: "item.listResult", items });
  ...
  const writeAuthoritativeClipboardText = vi.fn((value: Promise<string>) =>
    value.then(() => undefined),
  );
  const openVaultPage = vi.fn((): Promise<void> => Promise.resolve());
  const sendToTab = vi.fn(() =>
    Promise.resolve(
      options.fill ?? { version: 1, kind: "login.fillFromPopupResult", status: "filled" },
    ),
  );
...
describe("PopupApp home", () => {
  it("lists the categories with counts, loads the whole vault once, and passes axe", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("button", { name: /All items/ })).toHaveTextContent("3"),
    );
    ...
    expect(
      sendMessage.mock.calls.filter(
        ([payload]) => (payload as { kind?: unknown }).kind === "item.list",
      ),
    ).toHaveLength(1);
  ...
  it("suggests the logins saved for the open tab and fills from the row", async () => {
    ...
    const { sendToTab } = await renderUnlocked({
...
describe("PopupApp screens", () => {
  it("opens a category list, then an item's detail with copyable fields and a masked password", async () => {
    ...
    await waitFor(() =>
      expect(sendMessage).toHaveBeenCalledWith({ version: 1, kind: "item.get", itemId: LOGIN_ID }),
    );
  ...
  it("copies a login's password from its list row through login.reveal", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: /Logins/ }));
    fireEvent.click(
      await screen.findByRole("button", { name: "Copy password for Example Portal" }),
    );
    await waitFor(() =>
  ...
  it("generates a password from the footer and copies it", async () => {
    ...
    // The generator lets the options settle before asking (a slider drag is one request).
    await waitFor(() =>
      expect(screen.getByLabelText("Generated password")).toHaveTextContent("Tr0ub4dor-3xample-Q9"),
    );
    expect(sendMessage).toHaveBeenCalledWith(
      expect.objectContaining({ kind: "password.generate", mode: "random", length: 20 }),
    );
    expect(screen.getByText("96 bits of entropy")).toBeVisible();
    fireEvent.click(screen.getByRole("tab", { name: "Passphrase" }));
    await waitFor(() =>
      expect(sendMessage).toHaveBeenCalledWith(
        expect.objectContaining({ kind: "password.generate", mode: "passphrase", wordCount: 4 }),
      ),
    );
  ...
  it("stays on the unlocked screen and says so when locking fails", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "The vault could not be locked. Try again.",
    );
  ...
  it("gives search results the same quick actions as the lists", async () => {
    const { writeAuthoritativeClipboardText } = await renderUnlocked({
      tab: { id: 7, url: "https://example.test/login" },
    });
    fireEvent.change(screen.getByRole("searchbox", { name: "Search ShardPass" }), {
      target: { value: "north" },
    });
    ...
    fixture.sendMessage.mockImplementation((payload: unknown) => {
      ...
      if (request.kind === "item.list")
        return Promise.resolve({
          ...
          items: [loginProjection, ...identities],
        });
      return Promise.resolve(undefined);
    });
    try {
  ...
  it("shows which provider a login signs in with instead of an empty password", async () => {
    ...
    fixture.sendMessage.mockImplementation((payload: unknown) => {
      ...
      if (request.kind === "item.list")
        return Promise.resolve({
          version: 1,
          kind: "item.listResult",
          items: [{ ...loginProjection, signInWith: "google" }],
        });
      if (request.kind === "item.get")
        return Promise.resolve({ version: 1, kind: "item.getResult", item: viaGoogle });
      return Promise.resolve(undefined);
    });
    render(<PopupApp platform={fixture.platform} />);
    act(() => fixture.publishVaultState(vaultState("unlocked", 1)));
    fireEvent.click(await screen.findByRole("button", { name: /Logins/ }));
    expect(
      screen.queryByRole("button", { name: "Copy password for Example Portal" }),
