...
function platform(answer: (request: PasskeyRequest) => PasskeyResponse | Promise<PasskeyResponse>): PasskeyContentPlatform & { calls: PasskeyRequest[] } {
...
describe("passkey bridge", () => {
  it("previews the login, asks the person, registers with a clientDataJSON built from the real origin, and replies", async () => {
    const candidate = platform((request) => {
      if (request.kind === "passkey.preview")
        return { version: 1, kind: "passkey.previewResult", login: { itemId: "11111111-1111-4111-8111-111111111111", name: "Example", username: "me" } };
      ...
      window.postMessage(
        {
          ...
          id: "req-1",
          type: "create",
          request: { challenge: "AQID", rp: { id: null, name: "Example" }, user: { id: "CQ", name: "me", displayName: "Me" }, algorithms: [-7], excludeCredentialIds: [] },
        },
        "/",
      );
    ...
    expect(within(prompt).getByRole("heading", { name: "Create a passkey for localhost?" })).toBeVisible();
    ...
    const register = candidate.calls.find((call) => call.kind === "passkey.register") as Extract<PasskeyRequest, { kind: "passkey.register" }>;
    expect(register).toMatchObject({ rpId: "localhost", origin: window.location.origin, attachTo: "11111111-1111-4111-8111-111111111111", userName: "me" });
    const clientData = JSON.parse(atob(register.clientDataJson.replaceAll("-", "+").replaceAll("_", "/"))) as Record<string, unknown>;
    expect(clientData).toEqual({ type: "webauthn.create", challenge: "AQID", origin: window.location.origin, crossOrigin: false });
    expect(answers.seen.at(-1)).toMatchObject({ id: "req-1", result: { credentialId: "Y3JlZA", attestationObject: "AQI" } });
  ...
  it("offers the browser instead when the vault is locked, and when no passkey matches a get()", async () => {
    const roots = captureClosedRoots();
    const locked = platform(() => Promise.reject(Object.assign(new Error("locked"), { code: "VAULT_LOCKED" })));
    const bridge = createPasskeyBridge({ document, window, platform: locked });
    bridges.push(bridge);
    bridge.start();
    const answers = replies();
    await act(async () => {
      window.postMessage({ tag: "shardpass-passkey", direction: "request", id: "req-2", type: "get", request: { challenge: "AQ", rpId: null, allowCredentialIds: [] } }, "/");
      await new Promise((resolve) => setTimeout(resolve, 0));
    });
    await flush();
    const prompt = roots.at(-1) as unknown as HTMLElement;
    ...
    const empty = platform(() => ({ version: 1, kind: "passkey.candidatesResult", candidates: [] }));
    const second = createPasskeyBridge({ document, window, platform: empty });
    bridges.push(second);
    second.start();
    await act(async () => {
      window.postMessage({ tag: "shardpass-passkey", direction: "request", id: "req-3", type: "get", request: { challenge: "AQ", rpId: null, allowCredentialIds: [] } }, "/");
      await new Promise((resolve) => setTimeout(resolve, 0));
    });
    await flush();
