...
export function parseVaultPageHash(hash: string): VaultPageTarget | null {
  ...
  if (created !== null) {
    ...
    return (VAULT_ITEM_KINDS as readonly string[]).includes(kind) ? { newItem: kind as VaultItemKind } : null;
  }
  const item = /^\/item\/([0-9a-f-]{36})$/iu.exec(path);
  if (item !== null && item[1] !== undefined && ITEM_ID.test(item[1])) return { item: item[1].toLowerCase() };
  return null;
}
