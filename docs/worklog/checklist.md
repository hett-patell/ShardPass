...
- [x] The front door was the worst of it: the README opened "ShardPass is being rebuilt", counted the browser suite as "nine tests" when there are sixteen spec files, and said this machine only had Node 24 when it runs Node 22.14.0, the approved runtime. Rewritten to say what ShardPass is and where the changelog, privacy policy, store copy and architecture notes live, with the browser-suite count replaced rather than re-pinned so it cannot drift again. The Project 0 and Project 1 gate sections are unchanged -- `tests/security/docs.test.ts` pins phrases across those docs combined.
- [x] Checked the claims that could silently rot: `@noble/ciphers` and `@noble/hashes` are pinned at 2.2.0 in `pnpm-workspace.yaml`, exactly as `docs/security/cryptographic-format.md` says.
