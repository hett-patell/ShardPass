...
const requests = [
  ...
  { version: 1, kind: "login.saveOffer", domain: "example.com", username: "alice", password: "s3cret" },
] as const;
...
describe("login fill messaging", () => {
  it("accepts a well-formed suggestion", () => {
    ...
    expect(LoginFillSuggestionSchema.safeParse({ ...suggestion, expectedRevision: 0 }).success).toBe(
      false,
    );
    expect(LoginFillSuggestionSchema.safeParse({ ...suggestion, itemId: "not-a-uuid" }).success).toBe(
      false,
    );
  });
  ...
  it("rejects malformed requests", () => {
    expect(LoginFillSuggestionsRequestSchema.safeParse({ ...requests[0], domain: "" }).success).toBe(
      false,
    );
    expect(
    ...
    expect(LoginFillSelectRequestSchema.safeParse({ ...requests[1], expectedRevision: -1 }).success).toBe(
      false,
    );
    ...
    expect(LoginFillCancelRequestSchema.safeParse({ kind: "login.fillCancel" }).success).toBe(false);
    expect(SaveLoginOfferRequestSchema.safeParse({ ...requests[4], password: undefined }).success).toBe(
      false,
    );
  });
  ...
  it("accepts every response kind, with optional linkedOtpCode", () => {
    ...
    expect(
      LoginFillReleaseResponseSchema.safeParse({ ...responses[1], linkedOtpCode: "123456" }).success,
    ).toBe(true);
    expect(
