...
describe("matchLoginUrl", () => {
  it("domain: registrable-domain match including subdomains", () => {
    expect(matchLoginUrl("https://login.example.test/x", "https://example.test", "domain")).toBe(true);
  ...
  it("host: the exact host only, subdomains excluded", () => {
    expect(matchLoginUrl("https://login.example.test", "https://login.example.test/a", "host")).toBe(true);
    expect(matchLoginUrl("https://other.example.test", "https://login.example.test", "host")).toBe(false);
    expect(matchLoginUrl("https://example.test:8443", "https://example.test:8443", "host")).toBe(true);
  ...
  it("startsWith and exact compare the path, ignoring trailing slash and fragment", () => {
    expect(matchLoginUrl("https://e.test/app/login/", "https://e.test/app", "startsWith")).toBe(true);
    expect(matchLoginUrl("https://e.test/other", "https://e.test/app", "startsWith")).toBe(false);
    expect(matchLoginUrl("https://e.test/app/login#top", "https://e.test/app/login/", "exact")).toBe(true);
    expect(matchLoginUrl("https://e.test/app/login/2", "https://e.test/app/login", "exact")).toBe(false);
  });
  ...
  it("degrades path-sensitive modes to a host match when only a hostname is known", () => {
    expect(matchLoginUrl("login.example.test", "https://login.example.test/app/login", "exact")).toBe(true);
    expect(matchLoginUrl("other.example.test", "https://login.example.test/app", "startsWith")).toBe(false);
  });
