...
// Replace fast-srp-hap's Node-only SRP.genKey with WebCrypto random bytes.
function generateEphemeralKey(size = 32): Buffer {
...
export async function verifySRP(
  ...
  const client = generateSRPClient(attrs.srpSalt, attrs.srpUserID, loginSubKey);
